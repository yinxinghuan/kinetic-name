...
export default class extends THREE.Object3D {
  init(options) {
    ...
    if (/[^\x20-\x7e]/.test(this.opts.word)) {
  ...
  createCanvasText() {
    ...
    context.fillStyle = this.opts.color;
    context.textAlign = 'center';
    context.textBaseline = 'middle';
    context.font = '900 184px "PingFang SC", "Noto Sans CJK SC", sans-serif';
    context.fillText(this.opts.word, canvas.width / 2, canvas.height / 2, 960);

    const texture = new THREE.CanvasTexture(canvas);
    texture.minFilter = THREE.LinearFilter;
    texture.magFilter = THREE.LinearFilter;
    texture.needsUpdate = true;
    this.fontGeometry = new THREE.PlaneGeometry(2.1, 0.525);
    this.fontMaterial = new THREE.MeshBasicMaterial({
      map: texture,
      side: THREE.DoubleSide,
      transparent: true,
    });
    this.usesCanvasText = true;
  }
